    ...
    public Student getStudentFromStudentGroup(String firstName, String secondName){
        ...
        if(result.size() == 0){
            throw new IllegalStateException(
                    String.format("—тудент с именем %s и фамилией %s не найден", firstName, secondName)
            );
        }
        if(result.size() != 1){
            throw new IllegalStateException("Ќайдено более одного студента с указанными именем и фамилией");
        }
        return result.get(0);
    }
